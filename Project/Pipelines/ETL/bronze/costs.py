...
TARGET_TABLE = "bronze_costs"

# Create the target streaming table
dlt.create_streaming_table(
    name=f"{TARGET_CATALOG}.{TARGET_SCHEMA}.{TARGET_TABLE}",
    comment="Bronze layer: Consolidated costs data from all tables with 'cost' in the name"
)

# Get list of tables with "cost" in the name
tables_with_cost = [
    table.tableName 
    for table in spark.sql(f"SHOW TABLES IN {SOURCE_CATALOG}.{SOURCE_SCHEMA}").collect()
    if "cost" in table.tableName.lower()
]

# Create an append flow for each table with "cost" in the name
for table_name in tables_with_cost:
